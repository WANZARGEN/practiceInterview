...
```
  ...
  1. 개발자가 지정한 SQL, 저장프로시저 그리고 몇가지 고급 매핑을 지원하는 퍼시스턴스(persistence) 프레임워크이다.
     persistence - 데이터를 생성한 프로그램의 실행이 종료되더라도 사라지지 않는 데이터의 특성을 의미한다.
  2. 마이바티스는 JDBC로 처리하는 상당부분의 코드와 파라미터 설정 및 결과 매핑을 대신해준다.
```

### 8.	Node.js를 설명하세요.
```
  
  1. 구글 크롬의 자바스크립트 엔진(V8 Engine)에 기반해 만들어진 서버 사이드 플랫폼입니다.
  2. Node.js 는 코드를 실행할수 있는 하나의 방법인 JavaScript 런타임입니다.
  3. Node는 웹서버가 아닙니다. 아파치 웹서버 와 같이 HTML 파일 경로를 지정해주고 서버를 열고 그런 설정이 없습니다. (HTTP 서버를 직접 작성해야한다.)
  4. Paypal, Microsoft, eBay 등에서 사용하고 있습니다.
```
